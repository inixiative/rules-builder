...
import {
  ...
  useLensValuePicker,
} from '../src/schema/lensValuePicker';
...
const byPath = (opts: LensValuePickerOptions) => (out: ReturnType<typeof useLensValuePicker>) =>
...
describe('useLensValuePicker', () => {
  ...
  test('reaches relation-traversed paths as dotted paths at maxDepth', () => {
    ...
    const opts = byPath({ maxDepth: 1 })(result.current);
  ...
  test('memoizes on its input fields — stable ref for equal inputs, fresh ref when maxDepth changes', () => {
    const { result, rerender } = renderHook((opts: LensValuePickerOptions) => useLensValuePicker(lens, opts), {
