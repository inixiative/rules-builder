...
var presetSeed = (facet) => {
  const template = facet.condition;
  return mapSlots(
    template,
...
var fillVariables = (template, slots, node) => mapSlots(template, slots, ({ path }) => valueSourceOf(getNode(node, path)));
...
var canonical = (value) => {
  ...
  if (value && typeof value === "object") {
    ...
    for (const key of Object.keys(value).sort())
      ...
      else if (!isMetaKey(key)) out[key] = canonical(value[key]);
    return out;
  }
  return value;
};
      ...
      for (const slot of slots)
        for (const option of slot.variable.options ?? []) {
          ...
          if (!(0, import_json_rules2.checkRuleAgainstLens)(withOption, lens).ok)
            violations.push(
              `preset '${name}' option ${JSON.stringify(option)} at '${getNode(facet.condition, slot.path).field}' is not allowed by the lens`
            );
        }
      continue;
    }
...
var descend = (built, node, path) => {
  let b = built;
  let r = node;
  for (const seg of path) {
    if (!b || r === void 0) return void 0;
    if (typeof seg === "number") {
      if (b.kind !== "group") return void 0;
      b = b.children[seg];
      r = groupChildrenOf(r)[seg];
      continue;
    }
    if (seg !== "condition" && seg !== "filter") return void 0;
    if (b.kind !== "array") return void 0;
    const sub = b[seg];
    r = r[seg];
    b = sub && r !== void 0 && !isGroupNode(r) ? sub.children[0] : sub;
  }
  return b;
};
var presetVariables = (facet, node, built, ctx) => {
  const out = [];
  for (const slot of variableSlots(facet.condition)) {
    ...
    const valueLabels = field ? ctx.surfaceOpts.valueLabels?.[field] : void 0;
    out.push({
      path: slot.path,
      field,
...
var buildLeaf = (node, path, depth, ctx, scope) => {
  ...
  return leaf.atomic && leafMatch ? { ...leaf, variables: presetVariables(leafMatch, node, leaf, ctx) } : leaf;
};
var buildArray = (node, path, depth, ctx, scope) => {
  ...
  return built.atomic && matchedFacet ? { ...built, variables: presetVariables(matchedFacet, node, built, ctx) } : built;
};
var buildGroup = (node, path, depth, ctx, scope) => {
  ...
  return preset && matched ? { ...built, variables: presetVariables(matched, node, built, ctx) } : built;
};
