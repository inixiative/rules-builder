...
__export(src_exports, {
  ...
  lensScopeSurface: () => lensScopeSurface,
  ...
  useLensScopeSurface: () => useLensScopeSurface,
...
var isAll = (c) => isObj(c) && "all" in c && Array.isArray(c.all);
var isAny = (c) => isObj(c) && "any" in c && Array.isArray(c.any);
...
var getNode = (cond, path) => {
  ...
  for (const seg of path) {
    if (cur === void 0 || !isObj(cur)) return void 0;
...
var asGroupRoot = (cond) => cond !== void 0 && (isAll(cond) || isAny(cond)) ? cond : { all: cond !== void 0 ? [cond] : [] };
var normalizeGroups = (cond) => {
  if (!isObj(cond)) return cond;
  const rec = { ...cond };
  const kids = childArray(cond);
  if (kids) rec[isAll(cond) ? "all" : "any"] = kids.map(normalizeGroups);
  for (const key of ["condition", "filter"])
    if (rec[key] !== void 0) rec[key] = normalizeGroups(asGroupRoot(rec[key]));
  for (const key of ["if", "then", "else"])
    if (rec[key] !== void 0) rec[key] = normalizeGroups(rec[key]);
  return rec;
};
...
var groupMeta = (node) => {
  if (!isObj2(node)) return {};
  const { all: _all, any: _any, ...rest } = node;
  return rest;
};
var switchGroupOperator = (node, kind) => {
  if (!isObj2(node)) throw new Error("switchGroupOperator: not a compound");
  const rec = node;
...
var trimEmptyGroups = (node) => {
  if (!isObj2(node)) return node;
  const rec = node;
  const key = groupKey(rec);
  if (!key) {
    let next;
    for (const sub of SUB_CONDITION_KEYS) {
      if (!isObj2(rec[sub])) continue;
...
var withIds = (node, makeId = defaultMakeId) => {
  ...
  const next = { ...rec };
  for (const sub of SUB_CONDITION_KEYS) {
    if (isObj2(next[sub])) next[sub] = withIds(next[sub], makeId);
  }
...
var VALUE_SOURCE_KEYS = ["value", "path", "bind"];
var templateOf = (facet) => normalizeGroups(facet.condition);
var variableSlots = (template) => {
  const out = [];
  const walk = (node, path) => {
    if (!node || typeof node !== "object" || Array.isArray(node)) return;
    const rec = node;
    if (rec.variable !== void 0) out.push({ path, variable: rec.variable });
    const kids = Array.isArray(rec.all) ? rec.all : Array.isArray(rec.any) ? rec.any : void 0;
    if (kids) for (const [i, kid] of kids.entries()) walk(kid, [...path, i]);
    for (const key of ["condition", "filter", "if", "then", "else"])
      if (rec[key] !== void 0) walk(rec[key], [...path, key]);
  };
  walk(normalizeGroups(template), []);
  return out;
};
var mapSlots = (template, slots, replacement) => slots.reduce((acc, slot) => {
  const { variable: _v, ...rest } = getNode(acc, slot.path);
  return setNode(acc, slot.path, { ...rest, ...replacement(slot) });
}, template);
var maskAt = (template, paths) => paths.reduce((acc, path) => {
  const at = getNode(acc, path);
  if (!at || typeof at !== "object") return acc;
  const { variable: _v, value: _a, path: _p, bind: _b, ...rest } = at;
  return setNode(acc, path, rest);
}, template);
var valueSourceOf = (node) => {
  if (!node || typeof node !== "object") return {};
  const rec = node;
  const out = {};
  for (const key of VALUE_SOURCE_KEYS) if (rec[key] !== void 0) out[key] = rec[key];
  return out;
};
var presetSeed = (facet) => {
  const template = templateOf(facet);
  return mapSlots(
    template,
    variableSlots(template),
    ({ variable }) => variable.default !== void 0 ? { value: variable.default } : {}
  );
};
var fillVariables = (template, slots, node) => {
...
var STRUCTURAL_KEYS = /* @__PURE__ */ new Set(["all", "any", "condition", "filter", "if", "then", "else"]);
var canonical = (value, structural = true) => {
  ...
  if (value && typeof value === "object") {
    ...
    for (const key of Object.keys(rec).sort()) {
      ...
      if (structural && key === "variable") out[key] = {};
        ...
        out[key] = canonical(normalizeGroups(asGroupRoot(rec[key])), true);
...
var matchFacet = (lens, decoration, node) => {
  ...
  for (const facet of decoration.facets) {
    if (facet.condition === void 0) continue;
    const template = templateOf(facet);
    ...
    if (slots.length < bestPresetSlots || slots.length === bestPresetSlots && bestPreset && facetId(facet) < facetId(bestPreset)) {
      bestPreset = facet;
...
var validateFacetList = (lens, list, prefix) => {
  ...
  for (const facet of list) {
    if (facet.condition !== void 0) {
      ...
      if (!(0, import_json_rules2.checkRuleAgainstLens)(presetSeed(facet), lens).ok)
        violations.push(`preset '${name}' is not a valid rule against the lens`);
      const template = templateOf(facet);
      const slots = variableSlots(template);
      for (const slot of slots)
        for (const option of slot.variable.options ?? []) {
          const withOption = mapSlots(
            template,
            slots,
            (s) => s === slot ? { value: option } : {}
          );
          if (!(0, import_json_rules2.checkRuleAgainstLens)(withOption, lens).ok)
            violations.push(
              `preset '${name}' option ${JSON.stringify(option)} at '${getNode(template, slot.path).field}' is not allowed by the lens`
            );
        }
      for (const other of presets) {
        ...
        const nested = [...mine].every((p) => theirs.has(p)) || [...theirs].every((p) => mine.has(p));
        if (nested) continue;
...
var descend = (built, path) => {
  let b = built;
  for (const seg of path) {
    if (!b) return void 0;
    if (typeof seg === "number") b = b.kind === "group" ? b.children[seg] : void 0;
    else if (seg === "condition" || seg === "filter") b = b.kind === "array" ? b[seg] : void 0;
    else return void 0;
  }
  return b;
};
var presetVariables = (facet, built) => {
  const out = [];
  for (const slot of variableSlots(facet.condition)) {
    const target = descend(built, slot.path);
    const value = target?.kind === "leaf" ? target.value : target?.kind === "array" ? target.aggregate?.value : void 0;
    if (!target || !value) continue;
    const picker = target.kind === "leaf" ? target.field : target.kind === "array" ? target.aggregate?.field : void 0;
    const field = picker?.value;
    const named = target.kind === "leaf" ? target.value?.options : void 0;
    out.push({
      path: slot.path,
      field,
...
var buildLeaf = (node, path, depth, ctx, scope) => {
  ...
  const declared = subPath === void 0 ? field : void 0;
  const operators = subPath === void 0 ? field?.operators : genericOperators(ctx.surfaceOpts.targets);
  const operator = rec.dateOperator ?? rec.operator;
  const operatorOptions = operators ? [...operators.field, ...operators.date].map((o) => ({
    value: o,
    label: o
  })) : [];
  const shape = operator ? valueShapeForOperator(operator) : "none";
  const valueOptions = declared?.options ? declared.options.map((o) => ({
    value: o.value,
  ...
  const leaf = {
    ...
    operator: {
      ...
      set: (op) => {
        const isDate = operators?.date.includes(op) ?? false;
...
var buildArray = (node, path, depth, ctx, scope) => {
  ...
  const recognizable = !scope.decoration ? void 0 : isAggregate ? { ...scope.decoration, facets: scope.decoration.facets.filter(isPreset) } : scope.decoration;
  const matchedFacet = recognizable ? matchFacet(scope.lens, recognizable, node) : void 0;
    ...
    if (key === "condition" && matchedFacet) {
      ...
      if (lead > 0 && kids.length === lead + 1 && tail && isGroupNode(tail)) {
        ...
        selectorClauseNodes = clauseSlice.length ? clauseSlice.map((clause, i) => {
          const built2 = buildNode(clause, [whereLead + i], depth + 1, subCtx, subScope);
  ...
  return built.atomic && matchedFacet ? { ...built, variables: presetVariables(matchedFacet, built) } : built;
};
  ...
  if (identityLead > 0 && kids.length === identityLead + 1 && rowsTail && isGroupNode(rowsTail)) {
    ...
    return {
      ...
      selectorClauses: selectorGroupFacet && identityLead > whereLead ? kids.slice(whereLead, identityLead).map((clause, i) => {
        const built2 = buildNode(clause, [...path, whereLead + i], depth, ctx, groupScope);
        const clauseField = clause.field ?? groupChildrenOf(clause)[0]?.field;
        return clauseField ? { ...built2, remove: () => setGroupSelectorClause?.(clauseField, null) } : built2;
      }) : void 0,
...
// src/schema/lensScopeSurface.ts
...
var leafOption = (name, entry, path, enums, labels) => {
...
// src/schema/lensScopeSurface.ts
var RELATION_KINDS4 = /* @__PURE__ */ new Set(["object", "bridge"]);
var lensScopeSurface = (lensOrNarrowing, opts = {}) => {
  const lens = (0, import_json_rules7.exposedSurface)(lensOrNarrowing);
  const startMap = opts.mapName ?? lens.mapName;
  const startModel = opts.model ?? lens.model;
  const values = [];
  const loops = [];
  const walk = (mapName, modelName, prefix, seen) => {
    const model = lens.maps[mapName]?.models[modelName];
    if (!model) return;
    const key = `${mapName}:${modelName}`;
    if (seen.has(key)) return;
    ...
    for (const [name, entry] of Object.entries(model.fields)) {
      const path = prefix ? `${prefix}.${name}` : name;
      if (RELATION_KINDS4.has(entry.kind)) {
        const target = relationTarget(entry, mapName);
        if (!target) continue;
  ...
  walk(startMap, startModel, "", /* @__PURE__ */ new Set());
  return { values, loops };
};
var useLensScopeSurface = (lensOrNarrowing, opts = {}) => (
  // biome-ignore lint/correctness/useExhaustiveDependencies: depend on option fields, not opts identity, so inline literals don't re-run the walk
...
var import_json_rules8 = require("@inixiative/json-rules");
var runSources = (lensOrNarrowing, rows) => (0, import_json_rules8.sourceQueries)(lensOrNarrowing).map((q) => {
  const matched = (rows[q.model] ?? []).filter((r) => (0, import_json_rules8.check)(q.composedWhere, r) === true);
  const seen = /* @__PURE__ */ new Set();
  const options = [];
