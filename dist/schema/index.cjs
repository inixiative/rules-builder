...
var lensScopeSurface = (lensOrNarrowing, opts = {}) => {
  const lens = (0, import_json_rules4.exposedSurface)(lensOrNarrowing);
  const startMap = opts.mapName ?? lens.mapName;
  const startModel = opts.model ?? lens.model;
  const values = [];
  const loops = [];
...
var runSources = (lensOrNarrowing, rows) => (0, import_json_rules5.sourceQueries)(lensOrNarrowing).map((q) => {
  const matched = (rows[q.model] ?? []).filter((r) => (0, import_json_rules5.check)(q.composedWhere, r) === true);
  const seen = /* @__PURE__ */ new Set();
  const options = [];
